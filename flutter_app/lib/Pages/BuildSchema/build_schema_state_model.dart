...
class BuildSchemaStateModel extends ChangeNotifier {
  ...
  final TextEditingController _textEditingController =
      TextEditingController(text: "PC Build #1");
  ...
  String changeSelectedMotherboard(List query) {
    if (selectedCPU.isNotEmpty) {
      if (selectedCPU[0]['socket_name'] != query[0]['cpu_socket']) {
        return ('Incompatible cpu socket. cpu has ' +
            selectedCPU[0]['socket_name'] +
            ' while motherboard has ' +
            query[0]['cpu_socket']);
      }
    }
    ...
    if (selectedMotherboard.isNotEmpty) {
      if (query[0]['socket_name'] != selectedMotherboard[0]['cpu_socket']) {
        return ('Incompatible cpu socket. Cpu has ' +
            query[0]['socket_name'] +
            ' while motherboard has ' +
            selectedMotherboard[0]['cpu_socket']);
      }
    }
  ...
  String changeSelectedGPU(List query) {
    if (_currentSelectedMotherboard.isNotEmpty) {
      Map<String, dynamic> serializedJson =
          jsonDecode(_currentSelectedMotherboard[0]['pcie_slots_json']);
      if (CompatibilityCheckFunctions().handlePcieCompatibilityWithJson(
              query[0]['interface_bus'], serializedJson) ==
          false) {
        return ('Incompatible PCIE Slot. Gpu needs a ' +
            query[0]['interface_bus'] +
            ' slot');
      }
    }
    if (_currentSelectedPSU.isNotEmpty) {
      if (query[0]['recommended_wattage'] > _currentSelectedPSU[0]['power_W']) {
        return ('Not enough Power, needs at least ' +
            query[0]['recommended_wattage'].toString() +
            'W for the selected hardware.');
      }
    }
  ...
  String changeSelectedPSU(List query) {
    if (_currentSelectedGPU.isNotEmpty) {
      if (_currentSelectedGPU[0]['recommended_wattage'] > query[0]['power_W']) {
        return ('Not enough Power, needs at least ' +
            _currentSelectedGPU[0]['recommended_wattage'].toString() +
            'W for the selected hardware.');
      }
    }
  ...
  double calculatePrice() {
    ...
    if (selectedRAM.isNotEmpty) {
      initPrice +=
          selectedRAM[0]['ram_prices'][0]['price'] * currentSelectedRAMCount;
    }
    if (selectedStorage.isNotEmpty) {
