...
class CpuMotherboardPair {
  ...
  CpuMotherboardPair(
      Map<String, dynamic>? cpuInput, Map<String, dynamic>? motherboardInput) {
    cpuData = cpuInput;
    motherboardData = motherboardInput;

    if (cpuInput != null) {
      price +=
          cpuInput[GlobalVariables.getQueryPriceText(PartEnum.cpu)][0]['price'];
    }
    if (motherboardInput != null) {
      price += motherboardInput[
          GlobalVariables.getQueryPriceText(PartEnum.motherboard)][0]['price'];
    }
  }
}
...
class GpuPsuPair {
  ...
  GpuPsuPair(Map<String, dynamic>? gpuInput, Map<String, dynamic>? psuInput) {
    ...
    if (psuInput != null) {
      price +=
          psuInput[GlobalVariables.getQueryPriceText(PartEnum.psu)][0]['price'];
    }
    if (gpuInput != null) {
      price +=
          gpuInput[GlobalVariables.getQueryPriceText(PartEnum.gpu)][0]['price'];
    }
  }
}
...
class FullPcPartModel {
  ...
  FullPcPartModel(
    ...
    price = cpuMotherboardPairInput.price + gpuPsuPairInput.price;

    if (caseInput != null) {
      price += caseInput[GlobalVariables.getQueryPriceText(PartEnum.pcCase)][0]
          ['price'];
    }
    if (storageInput != null) {
      price += storageInput[GlobalVariables.getQueryPriceText(PartEnum.storage)]
          [0]['price'];
    }
    if (ramInput != null) {
      price += (ramInput[GlobalVariables.getQueryPriceText(PartEnum.ram)][0]
              ['price'] *
          ramCountInput);
    }
    if (coolerInput != null) {
      price += coolerInput[GlobalVariables.getQueryPriceText(PartEnum.cooling)]
          [0]['price'];
    }
  }
}
